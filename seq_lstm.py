...
from model.predictor import predict_lstm
...
if __name__ == "__main__":
    ...
    # build model
    print('loading model')
    ner_model = LSTM(len(f_map), len(l_map), jd['embedding_dim'], jd['hidden'], jd['layers'], jd['drop_out'])
    ...
    predictor = predict_lstm(if_cuda, f_map, l_map, f_map['<eof>'], l_map['<pad>'], start_label=None, label_seq=decode_label, batch_size=args.batch_size, caseless=jd['caseless'])
