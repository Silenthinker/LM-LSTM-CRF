...
import codecs
from model.crf import CRFRepack
from model.lstm import LSTM
import model.utils as utils
from model.evaluator import eval_lstm
...
if __name__ == "__main__":
    ...
    if args.load_check_point:
        ...
        train_features, train_labels, f_map, l_map = utils.generate_corpus(lines, if_shrink_feature=True, thresholds=0, start=False)
    ...
    # construct dataset
    dataset = utils.construct_bucket_mean_lstm(train_features, train_labels, f_map, l_map, args.caseless)
    dev_dataset = utils.construct_bucket_mean_lstm(dev_features, dev_labels, f_map, l_map, args.caseless)
    test_dataset = utils.construct_bucket_mean_lstm(test_features, test_labels, f_map, l_map, args.caseless)
    ...
    ner_model = LSTM(len(f_map), len(l_map), args.embedding_dim, args.hidden, args.layers, args.drop_out)
    ...
    # Loss and Optimizer
    crit = nn.CrossEntropyLoss() # do not reduce losses; using mask to filter losses
    ...
    evaluator = eval_lstm(packer, l_map, args.eva_matrix)
    if args.load_check_point:
    ...
    for epoch_idx, args.start_epoch in enumerate(epoch_list):
        ...
        for feature, tg, mask in tqdm(
            ...
            ner_model.zero_grad()
            outputs, hidden = ner_model.forward(fea_v)
            losses = crit.forward(outputs.view(-1, len(l_map)), tg_v.view(-1)) # reshape to (N, C)
            loss = torch.mean(losses*mask_v.view(-1).float())
            loss.backward()
